...
if($_POST['Submit'])
{
	...
	for($tm=1;$tm<=$tot_num;$tm++)
	{
		...
		$q_id=$_POST['q_id_'.$tm];
		$obj->qUpdate(TABLE_SURVEY_QUESTIONINFO,"question_order='".$seq."'","question_id='".$q_id."' and survey_id='".$survey_id."'");
	}
}
$res=$obj->selectData(TABLE_QUESTION." as a,".TABLE_SURVEY_QUESTIONINFO." as b","a.question_title,a.question_type,b.*","question_status='A' and survey_questioninfo_status='A' and a.question_id=b.question_id  and b.survey_id='".$_REQUEST['survey_id']."'","","survey_id,question_order");
                ...
                <tr>
                  <td align="left" valign="top" class="border2"><form name="form1" method="post" action="">
					  ...
					  <? if($numrows>0){?>
						...
						<? }else{?>
                        <tr class="tr_bg">
                          <td height="25" align="center" valign="top" class="error">No Info Question in this Survey!!!</td>
                        </tr>
						<? }?>
                      </table>
                  </form></td>
