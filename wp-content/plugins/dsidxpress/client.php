...
class dsSearchAgent_Client {
	...
	static function FrameAction($action, $get, $idx_page_id=false){
		...
		$description = NULL;
		$title = NULL;
		$script_code = '<script src="https://idx.diversesolutions.com/scripts/controls/Remote-Frame.aspx?MasterAccountID='. $options['AccountID'] .'&amp;SearchSetupID='. $options['SearchSetupID'] .'&amp;LinkID=0&amp;Height=2000"></script>';
	...
	static function SocialMetaTags() {
		...
		$firstimageWidth = self::$meta_tag_data['firstimageWidth'];
		$firstimageHeight = self::$meta_tag_data['firstimageHeight'];
		$title = self::$meta_tag_data['title'];
		$description = self::$meta_tag_data['description'];
		if (!empty($firstimage)) {
			echo "<meta property='og:image' content='" . $firstimage . ".jpg' />";
