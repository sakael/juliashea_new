  ...
  function getGoogleMapsAPIKey(){
     $apiHttpResponse = dsSearchAgent_ApiRequest::FetchData("AccountOptions", array(), false, 0);
		  if (!empty($apiHttpResponse["errors"]) || $apiHttpResponse["response"]["code"] != "200")
			  echo "<script type='text/javascript'>console.log('Google Map API Key Not Set');</script>";
		  else
		...
		if ($hook == 'nav-menus.php' || $hook == 'post.php' || $hook == 'post-new.php') {
			...
			if (empty($apiHttpResponse["errors"]) && $apiHttpResponse["response"]["code"] == "200") {
				if(isset($apiHttpResponse['body'])) {
					$account_options = json_decode($apiHttpResponse["body"]);
					$dsIDXPressPackage = isset($account_options->{'dsIDXPress-Package'})? $account_options->{'dsIDXPress-Package'}:'';
			...
			wp_localize_script('dsidxpress_admin_utilities', 'dsIDXPressPackage', $dsIDXPressPackage);
	...
	static function DisplayAdminNotices() {
		...
		if (!isset($options["PrivateApiKey"])) { ?>
			<div class="error">
					...
					<?php if(htmlspecialchars($_GET["page"])!="dsidxpress"){ ?>
...
HTML;
	...
	static function DisplayDeveloperNotice() {
		...
		if ($pagenow != "index.php")
			if (!isset($_GET["page"]) || (isset($_GET["page"]) && stripos($_GET["page"], "dsidxpress") === false))
				if (!isset($_GET["post_type"]) || (isset($_GET["post_type"]) && stripos($_GET["post_type"], "ds-idx") === false))
					return;
		$options = get_option(DSIDXPRESS_OPTION_NAME);
		...
		<h2>Display Settings</h2>
			<table class="form-table">
				...
				<tr>
					...
					<td>
						<input type="checkbox" id="dsidxpress-UseAcresInsteadOfSqFt-check" class="dsidxpress-api-checkbox" <?php checked('true', strtolower($account_options->UseAcresInsteadOfSqFt)); ?> /><br />
						<input type="hidden" id="dsidxpress-UseAcresInsteadOfSqFt" name="<?php echo DSIDXPRESS_API_OPTIONS_NAME; ?>[UseAcresInsteadOfSqFt]" value="<?php echo $account_options->UseAcresInsteadOfSqFt;?>" />
...
<?php
	...
	static function SEOSettings() {
		...
		if(isset($account_options->dsIDXPressSEODetailsLinkTract)) {			
			if($account_options->dsIDXPressSEODetailsLinkTract=='true')
				$linkTractName = 'checked';
			else 
				$linkTractName = '';	
		}		
		if(isset($account_options->dsIDXPressSEODetailsLinkCommunity)) {
			if( $account_options->dsIDXPressSEODetailsLinkCommunity=='true')
			...
			<table class="form-table">
				...
				<th><label for="dsidxpress-DetailsTitle">Tracts:</th>
					<td>
						...
						name="<?php echo DSIDXPRESS_API_OPTIONS_NAME; ?>[dsIDXPressSEODetailsLinkTract]"/>
						<input type="checkbox"  
						 id="dsIDXPressSEODetailsLinkTractCB" class="dsidxpress-api-checkbox"  onclick="dsIDXpressOptions.OptionCheckBoxClick(this);"
				...
				<th><label for="dsidxpress-DetailsTitle">Communities:</th>
					...
					<input type='hidden'  id="dsIDXPressSEODetailsLinkCommunity" value='<?php echo ($linkCommunityName!=''?'true':'false');?>'
						name="<?php echo DSIDXPRESS_API_OPTIONS_NAME; ?>[dsIDXPressSEODetailsLinkCommunity]"/>
		...
		<div class="wrap metabox-holder">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					...
					<?php $defaultInterestRate = isset($account_options->{'DefaultInterestRate'})? $account_options->{'DefaultInterestRate'}:''; ?>
					<tr>
						<th>
						...
						<td>
							...
							name="<?php echo DSIDXPRESS_API_OPTIONS_NAME; ?>[DefaultInterestRate]" 
					...
					<tr>
						...
						<td>
							<input type="checkbox" id="dsidxpress-EnableThirdPartyLoginsCB" size="50" <?php checked('true', strtolower($account_options->EnableThirdPartyLogins)); ?> onclick="dsIDXpressOptions.OptionCheckBoxClick(this);" /><br />
							<input type="hidden" id="dsidxpress-EnableThirdPartyLogins" name="<?php echo DSIDXPRESS_API_OPTIONS_NAME; ?>[EnableThirdPartyLogins]" value="<?php echo $account_options->EnableThirdPartyLogins;?>" />
							<span class="description"></span>
						</td>
					</tr>
				</table>
	...
	public static function LinkBuilderHtml($in_post_dialog = false, $_nav_menu_placeholder = -1, $nav_menu_selected_id = 1, $in_idx_page_options=false, $preset_url='') {
		...
		$url_value   = ($in_post_dialog && isset($_GET['selected_url'])) ? htmlspecialchars($_GET['selected_url']) : 'https://';
