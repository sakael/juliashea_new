<?php
        ...
        if( !empty($post_meta_data['REAL_HOMES_property_garage'][0]) ) {
                ...
                $garage_label = ($prop_garage > 1)?__('Car Garage','framework' ): __('Car Garage','framework');
		  ...
		  if( !empty($post_meta_data['REAL_HOMES_property_area'][0]) ) {

                $area_bathrooms = $post_meta_data['REAL_HOMES_property_area'][0];

                $area_label ='Area';// ($area_bathrooms > 1)?__('Area','framework' ): __('Area','framework');

                echo '<span><i class="icon-location"></i>'.$area_label.'&nbsp;:&nbsp;'. $area_bathrooms .'</span>';
