...
class simvance_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'simvance_rp', // Base ID
			__('Simvance Recent Posts', 'simvance'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'simvance' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'simvance' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'simvance' );
		}
		?>
		<p>
...
add_action( 'widgets_init', 'register_simvance_widget' );  
function register_simvance_widget() {  
    register_widget( 'Simvance_Recent_Posts' );  
}  
